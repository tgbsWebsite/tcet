...
export default function EventCalendar() {
  ...
  const daysInPrevMonth = useMemo(() => new Date(viewYear, viewMonth, 0).getDate(), [viewYear, viewMonth]);
  ...
  useEffect(() => {
    ...
    setSelectedDate({ key: todayKey, date: d, hasEvent: Boolean(events[todayKey]) });
  }, [now, events]);
  ...
  const renderCurrent = () => {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      const classNames = ['ec-day', hasEvent ? 'ec-day--hasEvent' : '', isToday ? 'ec-day--today' : '', isSelected ? 'ec-day--selected' : '']
        .filter(Boolean)
        .join(' ');
      ...
      cells.push(
        <button
          ...
          aria-label={`${d} ${EC_MONTHS[viewMonth]} ${viewYear}${hasEvent ? ' — has event' : ''}`}
        >
          {d}
        </button>
      );
        ...
        <section className="ec-calendarSection">
          <div className="ec-calendar">
            <div className="ec-nav">
              <button className="ec-navBtn" type="button" onClick={previousMonth} aria-label="Previous month">
                ‹
              </button>
              <div className="ec-monthYear" aria-live="polite">
                {EC_MONTHS[viewMonth]} {viewYear}
              </div>
              <button className="ec-navBtn" type="button" onClick={nextMonth} aria-label="Next month">
                ›
              </button>
            </div>

            <div className="ec-grid" role="grid" aria-labelledby="ec-grid-title">
              <div id="ec-grid-title" className="ec-gridTitle" aria-hidden="true">
                CALENDAR
              </div>
              {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((d) => (
                <div key={d} className="ec-dayHeader" role="columnheader" aria-label={d}>
                  {d}
                </div>
              ))}
        ...
        <section className="ec-eventSection">
          ...
          <div className="ec-card">
            <div className="ec-cardMedia">
              ...
              />
              <span className="ec-cardBadge">{selectedDetails ? selectedDetails.label : 'Date'}</span>
            </div>
            <div className="ec-cardBody">
              <h3 className="ec-cardTitle">{selectedDetails ? selectedDetails.name : 'EVENT NAME'}</h3>
